...
describe('RefreshTokenGuard', () => {
  ...
  describe('handleRequest', () => {
    ...
    it('should throw UnauthorizedException when error is provided', () => {
      ...
      expect(() => guard.handleRequest(error, mockUser)).toThrow('Invalid refresh token');
    });

    it('should throw UnauthorizedException when user is null', () => {
      expect(() => guard.handleRequest(null, null)).toThrow(UnauthorizedException);
      expect(() => guard.handleRequest(null, null)).toThrow('Invalid refresh token');
    });

    it('should throw UnauthorizedException when user is undefined', () => {
      expect(() => guard.handleRequest(null, undefined)).toThrow(UnauthorizedException);
      expect(() => guard.handleRequest(null, undefined)).toThrow('Invalid refresh token');
    });
    ...
    it('should throw UnauthorizedException when both error and no user', () => {
      ...
      expect(() => guard.handleRequest(error, null)).toThrow('Invalid refresh token');
    });
  });
});
